...
Delivery OTP messages use these same `EMAIL_*` settings. The customer account
must have a valid email address. In production, delivery start is rejected if
mail credentials or the customer email are missing, so an order cannot enter
