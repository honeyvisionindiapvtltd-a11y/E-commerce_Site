  ...
  try {
    ...
    if (!user) {
      user = new User({
        ...
        status: 'Active',
        emailVerified: true,
        profile: { fullName: payload.name || '', email: normalizedEmail, country: 'India' },
      });
      user.setPassword(crypto.randomBytes(32).toString('hex'));
    } else {
      user.emailVerified = true;
      if (payload.name && (!user.name || user.name === normalizedEmail)) user.name = payload.name;
      user.profile = { ...(user.profile || {}), fullName: payload.name || user.name, email: normalizedEmail };
    }
    await user.save();
    res.json(createAuthResponse(user));
  } catch (error) {
...
router.get('/customers', authMiddleware, requireAdmin, async (req, res) => {
  ...
  const customers = users.map((user) => ({
    id: user._id.toString(),
    name: user.profile?.fullName || user.name || 'Unknown Customer',
