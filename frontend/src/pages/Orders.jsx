...
export default function Orders() {
  const { user, orders } = useCommerce();
  const [allOrders, setAllOrders] = useState([]);
  const [filteredOrders, setFilteredOrders] = useState([]);
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [filterStatus, setFilterStatus] = useState('all');
  const { subscribeToNotifications } = useRealtimeUpdates(user?.id);
  const { info } = useNotifications();

  const ownOrders = user ? orders.filter((order) => order.userId === user.id) : [];
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${API_BASE}/tracking/user/${user.id}`);
  ...
  useEffect(() => {
    ...
    // Filter by status
    if (filterStatus !== 'all') {
      filtered = filtered.filter((order) => (order.orderStatus || order.status) === filterStatus);
    }
            ...
            {!loading && filteredOrders.length === 0 ? (
              ...
              <div className="space-y-5">
                {filteredOrders.map((order) => (
                  <article key={order.id || order.orderId} className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm hover:shadow-md transition">
                    <Link
                      to={`/tracking?order=${encodeURIComponent(order.orderNumber || order.id || order.orderId)}`}
                      className="block"
                    >
                      <div className="flex flex-col gap-5 border-b border-gray-100 pb-5 md:flex-row md:items-center md:justify-between">
                        <div className="flex-1">
                          <p className="text-sm uppercase tracking-[0.16em] text-slate-400">Order #{order.id || order.orderId}</p>
                          <h2 className="mt-2 text-xl font-bold text-[#071426] capitalize">{(order.orderStatus || order.status).replace(/_/g, ' ')}</h2>
                        </div>
