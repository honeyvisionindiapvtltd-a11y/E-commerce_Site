...
import { adminListNotifications } from "../api";
...
import {
  ...
  Package, Percent, Settings, ShieldCheck, ShoppingCart, Star, Truck, Wrench,
  Home, User, Users, X
} from "lucide-react";
...
const groups = [
  ...
  {title:"OPERATIONS", items:[["Delivery","/admin/delivery",Truck],["Installations","/admin/installations",Wrench],["Services","/admin/services",Wrench],["Payments","/admin/payments",CreditCard]]},
...
export default function AdminLayout() {
  ...
  const { subscribeToAdminNotifications } = useRealtimeUpdates("admin");
  ...
  const loadNotifications = async () => {
    try { setNotificationItems(await adminListNotifications()); } catch { setNotificationItems([]); }
  };
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAdminNotifications((notification) => {
      setNotificationItems((current) => [{ ...notification, id: notification.id || Date.now() }, ...current].slice(0, 10));
    });
    return unsubscribe;
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-58.75 flex-col bg-[#031426] text-white transition-transform lg:translate-x-0 ${open?"translate-x-0":"-translate-x-full"}`}>
      ...
      <div className="border-t border-white/10 p-3">
        ...
        <button onClick={() => navigate("/")} className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-xs text-slate-300 hover:bg-white/10"><Home size={15}/>View Website</button>
      </div>
    </aside>

    <div className="lg:pl-58.75 min-h-screen">
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 shadow-sm sm:px-6">
        ...
        <div className="flex items-center gap-2">
          <button className="relative rounded-lg p-2 hover:bg-slate-100" onClick={toggleNotifications}><Bell size={18}/>{notificationItems.length > 0 && <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-red-500"/>}</button>
          <button className="hidden rounded-lg p-2 hover:bg-slate-100 sm:block"><HelpCircle size={18}/></button>
          <button title="View Website" onClick={() => navigate("/")} className="hidden rounded-lg p-2 hover:bg-slate-100 sm:block"><Home size={18}/></button>
      ...
      {notifications && <div className="fixed right-4 top-20 z-40 w-80 rounded-xl border bg-white p-4 shadow-xl">
        <b className="text-sm">Notifications</b>
        <div className="mt-3 max-h-80 space-y-2 overflow-y-auto text-xs text-slate-600">{notificationItems.length ? notificationItems.map((item) => <p key={item.id} className={`rounded-lg p-3 ${item.level === "warning" ? "bg-amber-50 text-amber-800" : "bg-slate-50"}`}>{item.message}</p>) : <p className="rounded-lg bg-slate-50 p-3">No notifications</p>}</div>
      </div>}
